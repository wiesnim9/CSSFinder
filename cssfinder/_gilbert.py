...
from cssfinder.report import makeshortreport
from cssfinder.types import MatrixC128T, MatrixT
...
class Gilbert:
    ...
    def run(self, visibility: float, steps: int, corrections: int) -> None:
        ...
        logging.critical("Optimization took {:.0f}s", end_time - start_time)
...
def _gilbert(
    ...
    _debug_msg_short_rho(True, 0, input_state)

    rho1 = np.zeros_like(input_state, dtype=np.complex128)
    np.fill_diagonal(rho1, input_state.diagonal())

    _debug_msg_short_rho(True, 1, rho1)

    rho3 = input_state - rho1
    ...
    product_0_1 = ops.product(input_state, rho1)
    ...
    for idx in range(steps):
        ...
        if len(corrections_list) >= corrections:
            break

        if corrections_list and corrections_list[-1].value <= precision:
            break
        ...
        if product_2_3 > limiter_product_1_3:
            ...
            product_0_2 = ops.product(input_state, rho2)
            ...
            if 0 < cc1 <= 1:
                ...
                corrections_list.append(
                    Correction(idx, correction_counter, ops.product(rho3, rho3))
                )

    logger.info("Finished optimization...")
    with open("list.txt", "w") as file:
        print(len(corrections_list), file=file)
        for c in corrections_list:
            print(c, file=file)

    makeshortreport(corrections_list)
